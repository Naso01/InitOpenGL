# InitOpenGL - `Lab4-3_TexturesFilteringMipMaps`

This repository is a **Visual Studio C++ OpenGL** course project. Work is organized across **many Git branches**; you are viewing **`Lab4-3_TexturesFilteringMipMaps`**. For the **complete branch index**, lab sequence, build notes, and lighting overview, see the canonical README on **`master`**.
...
Iteration on **Lab4-2**: refines **`Mesh`**, **`Shader`**, **`Texture`**, and **`SimpleFragmentShader`** so texture parameters and draw paths stay consistent.
